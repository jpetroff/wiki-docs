<script lang="ts">
  ...
  import { documentUrl, type DirectoryNode } from '$lib/shared/navigation';
...
<section data-documentation-mode="files" class="space-y-6">
  <div class="flex items-center justify-between gap-4">
    <h1 class="break-words text-3xl font-semibold">{directory.name || title}</h1>
    {#if canEdit}<CreateMenu path={directory.path} name={directory.name || title} />{/if}
  </div>
    ...
    <ul class="divide-y rounded-lg border">
      {#each directory.children as entry (entry.path)}
        <li class="flex items-center gap-3 px-4 py-3">
          ...
          <a class="min-w-0 flex-1 break-words hover:underline" href={documentUrl(entry.path)} onclick={() => { if (entry.kind === 'directory' && entry.hasChildren) void navigation.expand(entry.path); }}>{entry.name}</a>
          {#if canEdit && entry.kind === 'directory'}<CreateMenu path={entry.path} name={entry.name} />{/if}
        </li>
      {/each}
    </ul>
